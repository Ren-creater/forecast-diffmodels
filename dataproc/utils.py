    ...
    def get_batch(self, idx): 
        ...
        if self.mode == "fc":
            if self.modality == "vid":
                return self._to3channel(self.vid_cond[idx]).unsqueeze(2), self.vid_to3channel(self.vid[idx]), self.era5_vid[idx]
            else:
                return self._to3channel(self.img_cond[idx]).unsqueeze(2), self._to3channel(self.img[idx]).unsqueeze(2), self.zero_pad(self.era5_img[idx])
            #return self._to3channel(self.img_o[idx]), self.img_n[idx], self.era5[idx]
