...
def get_satmaps(region, name):
    ...
    if region == "nio":
        IR108_BASE_DIR = "/vol/bitbucket/zr523/satellite/mosdac"
    ...
    if region in ["aus", "wpo"]:
        IR108_BASE_DIR = "/vol/bitbucket/zr523/satellite/himawari"
    ...
    if region == "wio":
        IR108_BASE_DIR = "/vol/bitbucket/zr523/satellite/msg"
    ...
    if region == "use":
        IR108_BASE_DIR = "/vol/bitbucket/zr523/satellite/goes_east"
...
if region == "nio":  
    # names = sorted([x.split('/')[-1] for x in glob.glob("/vol/bitbucket/zr523/satellite/mosdac/data/h5/*")])
    names = ["gulab-shaheen"]
if region == "aus":
    names = ["ilsa", "seroja", "niran", "damien", "ferdinand", "veronica"]
if region == "wio":
    names = sorted([x.split('/')[-1] for x in glob.glob("/vol/bitbucket/zr523/satellite/msg/data/native/*")])
if region == "use":
    names = sorted([x.split('/')[-1] for x in glob.glob("/vol/bitbucket/zr523/satellite/goes_east/data/nc/*")])
if region == "usw":
    names = sorted([x.split('/')[-1] for x in glob.glob("/vol/bitbucket/zr523/satellite/goes_west/data/nc/*")])
if region == "wpo":
