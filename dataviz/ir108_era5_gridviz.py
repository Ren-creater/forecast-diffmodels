...
if region == "nio":  
    # names = sorted([x.split('/')[-1] for x in glob.glob("/rds/general/user/zr523/home/researchProject/satellite/mosdac/data/h5/*")])
    names = ["gulab-shaheen"]
if region == "aus":
    names = ["ilsa", "seroja", "niran", "damien", "ferdinand", "veronica"]
if region == "wio":
    names = sorted([x.split('/')[-1] for x in glob.glob("/rds/general/user/zr523/home/researchProject/satellite/msg/data/native/*")])
if region == "use":
    names = sorted([x.split('/')[-1] for x in glob.glob("/rds/general/user/zr523/home/researchProject/satellite/goes_east/data/nc/*")])
if region == "usw":
    names = sorted([x.split('/')[-1] for x in glob.glob("/rds/general/user/zr523/home/researchProject/satellite/goes_west/data/nc/*")])
if region == "wpo":
